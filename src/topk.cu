 ...
 * @param query the pointer to the query vector bitset.
...
void __global__ docQueryScoringCoalescedMemoryAccessSampleKernel(
  ...
  __shared__ uint32_t query_on_shm[MAX_ELEMENT];
#pragma unroll
  for (int l = threadIdx.x; l < MAX_ELEMENT; l += N_THREADS_IN_ONE_BLOCK) {
    query_on_shm[l] = __ldg(query + l);
  }
  __syncthreads();
...
#pragma unroll
    for (uint8_t k = 0; k < ROLL_INNER; ++k) {
      index = docs[tid * MAX_DOC_SIZE + j + k] >> 5;
      offset = docs[tid * MAX_DOC_SIZE + j + k] & 31;
      tmp_score += (j + k < doc_len) & ((query_on_shm[index] >> offset) & 1UL);
    }
  }
  //   __syncthreads();
...
void doc_query_scoring_gpu_function(std::vector<std::vector<uint16_t>>& querys,
  ...
  */
  ...
  std::vector<uint32_t*> query_data(query_num, nullptr);
   ...
   *  threads vector, cudaStreams, vectors and query data(query bitset) copy.
   */
  threads.reserve(BATCH_NUM);
  for (int i = 0; i < BATCH_NUM; ++i) {
    cudaStreamCreateWithFlags(&batch_streams[i], cudaStreamNonBlocking);
  }

  std::vector<uint32_t> query_mask(MAX_ELEMENT, 0u);
  for (int i = 0; i < query_num; ++i) {
    cudaMalloc(&query_data[i], sizeof(uint32_t) * MAX_ELEMENT);
    query_mask = std::vector<uint32_t>(MAX_ELEMENT, 0u);
    for (auto& id : querys[i]) {
      int index = id / 32;
      int offset = id % 32;
      query_mask[index] |= ((1u) << offset);
    }
    cudaMemcpy(query_data[i], query_mask.data(), sizeof(uint32_t) * MAX_ELEMENT,
