...
void doc_query_scoring_gpu_function(
  ...
  // for (int batch_id = 0; batch_id < BATCH_NUM/2; batch_id++) {
  //   threads.emplace_back([&, batch_id]() {
  //     int id = 2 * batch_id;
  //     cudaHostAlloc(&doc_data_per_batch[id],
  //                   sizeof(uint16_t) * MAX_DOC_SIZE * BATCH_SIZE,
  //                   cudaHostAllocPortable);
  //     cudaHostAlloc(&doc_len_per_batch[id], sizeof(int) * BATCH_SIZE,
  //                   cudaHostAllocPortable);
  //     h_batch_alloc[id] = true;
  //     cudaHostAlloc(&doc_data_per_batch[id+1],
  //                   sizeof(uint16_t) * MAX_DOC_SIZE * BATCH_SIZE,
  //                   cudaHostAllocPortable);
  //     cudaHostAlloc(&doc_len_per_batch[id+1], sizeof(int) * BATCH_SIZE,
  //                   cudaHostAllocPortable);
  //     h_batch_alloc[id+1] = true;
  //   });
  // }
  ...
  for (done_docs_n = 0; done_docs_n < n_docs;) {
    ...
    if (h_batch_alloc[batch_id] == false) {
      cudaHostAlloc(&doc_data_per_batch[batch_id],
                    sizeof(uint16_t) * MAX_DOC_SIZE * BATCH_SIZE,cudaHostAllocDefault);
      cudaHostAlloc(&doc_len_per_batch[batch_id], sizeof(int) * BATCH_SIZE,cudaHostAllocDefault);
      // doc_data_per_batch[batch_id]= (uint16_t*)malloc( sizeof(uint16_t) * MAX_DOC_SIZE * BATCH_SIZE);
      // doc_len_per_batch[batch_id] = (int*)malloc(sizeof(int) * BATCH_SIZE);
  ...
  // t3 = system_clock::now();
  int last_batch_id = batch_id == -1 ? (BATCH_NUM -1) : (batch_id-1)%BATCH_NUM;
  cudaStreamSynchronize(batch_streams[last_batch_id]);
  // for (int i = 0 ; i < BATCH_NUM; ++i) {
  //   cudaStreamSynchronize(batch_streams[batch_id]);
  // }

  cudaMemcpy(thrust::raw_pointer_cast(query_ranks[0].data()),
  ...
  for (int query_id = 1; query_id < query_num; ++ query_id) {
      cudaMemcpy(thrust::raw_pointer_cast(query_ranks[query_id].data()),
               thrust::raw_pointer_cast(query_ranks[0].data()), n_docs * sizeof(int), cudaMemcpyDeviceToDevice);
  }
  for (int query_id = 0; query_id < query_num; ++query_id) {
    // cudaMemcpy(thrust::raw_pointer_cast(query_ranks[query_id].data()),
    //            s_indices, n_docs * sizeof(int), cudaMemcpyHostToDevice);
